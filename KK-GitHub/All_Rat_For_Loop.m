...
% 
% Window: window around event in seconds - update x axis in stackPSTH and
% raster to reflect window size
% 
% bin: selected bin size
                    ...
                    if out == 1
                        ...
                        if ~isempty(tokens)
                            ...
                            if ~isempty(intan_data)
                                [spikeStruct, clu, st, unique_clusters, numSpks, cluster_spikes, valid_trials, valid_trial_flags, ts] = import_all_data(intan_folder, trials, 'correct', 'sideIn');
                                ...
                                if ~isempty(cluster_spikes)
                                    for g = 1:size(cluster_spikes, 1)
                                        ...
                                        RAT_SESSION_UNITNAME = strcat(session_info.ratID, '_', session_info.date, '_', session_info.session_letter, '_', 'correct', '_', 'sideIn', num2str(unique_clusters(g))); %establishing label for output
                                        StackPSTHandRaster(bins, psthHz, rasterX, rasterY, RAT_SESSION_UNITNAME);
